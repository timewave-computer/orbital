    ...
    fn from(chains: ChainsVec) -> Self {
        ...
        for chain in chains.chains {
            ...
            let (src_addr, denom) = match rb.chain_id.as_str() {
                ...
                JUNO_CHAIN_ID => ("juno1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl", "ujuno"),
                _ => ("err", "err"),
            };
            let local_chain =
        ...
        let juno_channels = chains_map.get(JUNO_CHAIN).unwrap().channels.clone();
        ...
        let (ntrn_to_juno_transfer_channel, juno_to_ntrn_transfer_channel) =
            find_pairwise_transfer_channel_ids(&ntrn_channels, &juno_channels).unwrap();

        connection_ids.insert(
            (NEUTRON_CHAIN.to_string(), JUNO_CHAIN.to_string()),
            ntrn_to_juno_transfer_channel.connection_id,
        );
        connection_ids.insert(
            (JUNO_CHAIN.to_string(), NEUTRON_CHAIN.to_string()),
            juno_to_ntrn_transfer_channel.connection_id,
        );

        let mut transfer_channel_ids = HashMap::new();
        transfer_channel_ids.insert(
            (NEUTRON_CHAIN.to_string(), JUNO_CHAIN.to_string()),
            ntrn_to_juno_transfer_channel.channel_id.to_string(),
        );
        transfer_channel_ids.insert(
            (JUNO_CHAIN.to_string(), NEUTRON_CHAIN.to_string()),
            juno_to_ntrn_transfer_channel.channel_id.to_string(),
        );
        transfer_channel_ids.insert(
            (NEUTRON_CHAIN.to_string(), GAIA_CHAIN.to_string()),
        ...
        ibc_denoms.insert(
            (NEUTRON_CHAIN.to_string(), JUNO_CHAIN.to_string()),
            utils::ibc::get_ibc_denom("untrn", &ntrn_to_juno_transfer_channel.channel_id),
        );
        ibc_denoms.insert(
            (JUNO_CHAIN.to_string(), NEUTRON_CHAIN.to_string()),
            utils::ibc::get_ibc_denom("ujuno", &juno_to_ntrn_transfer_channel.channel_id),
        );
        ibc_denoms.insert(
            (NEUTRON_CHAIN.to_string(), GAIA_CHAIN.to_string()),
